...
function AdministracionPanelEmpresa() {
  ...
  const secciones = [
    ...
    {
      id: "prestamo",
      nombre: "Guía de Extintores en calidad de prestamo",
      icono: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
    ...
    {
      id: "certificado",
      nombre: "Certificado de Operatividad",
      icono: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {secciones.map((seccion, index) => (
            <motion.button
              ...
              className={`group relative flex items-center gap-3 p-4 rounded-xl font-medium transition-all duration-300 transform hover:scale-105 ${subseccion === seccion.id
                  ? "bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg shadow-red-500/25"
                  : "bg-gray-50 text-gray-700 hover:bg-gray-100 hover:shadow-md"
                }`}
            >
              <div
                className={`flex-shrink-0 transition-colors duration-300 ${subseccion === seccion.id ? "text-white" : "text-gray-500 group-hover:text-gray-700"
                  }`}
              >
                {seccion.icono}
              </div>
