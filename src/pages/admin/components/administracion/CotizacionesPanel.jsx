...
function CotizacionesPanel({ empresaSeleccionada }) {
  ...
  useEffect(() => {
    const obtenerArchivos = async () => {
      ...
      const q = query(collection(db, "cotizaciones"), where("empresaId", "==", empresaSeleccionada.id))
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const archivoRef = ref(storage, `cotizaciones/${empresaSeleccionada.id}/${archivoSeleccionado.name}`)
  ...
  const confirmarEliminarArchivo = async () => {
    ...
    try {
      await deleteDoc(doc(db, "cotizaciones", archivoAEliminar.id))
  ...
  const obtenerBadgeTipo = (tipo) => {
    switch (tipo) {
      ...
      case "presupuesto":
        return (
          <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">Presupuesto</span>
        )
      case "propuesta":
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">Propuesta</span>
      default:
    ...
    >
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-6 mb-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Panel de Cotizaciones</h1>
            <p className="text-red-100">Gestiona las cotizaciones y presupuestos de la empresa</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-6">
          ...
          <div className="relative">
            <textarea
              placeholder="Agrega un comentario sobre esta cotización..."
              value={comentario}
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {["todos", "cotizacion", "presupuesto", "propuesta", "documento"].map((tipo) => (
                <button
                  key={tipo}
        ...
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500">Esta empresa aún no tiene cotizaciones subidas.</p>
            </div>
          </div>
        </motion.div>
      ) : (
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Cotización subida correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalExito(false)}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¡Éxito!</h3>
                  <p className="text-sm text-gray-500">Cotización eliminada correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalEliminado(false)}
