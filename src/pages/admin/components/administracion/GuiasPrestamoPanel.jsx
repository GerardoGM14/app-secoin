// src/pages/admin/components/administracion/GuiasPrestamoPanel.jsx
"use client"
...
function GuiasPrestamoPanel({ empresaSeleccionada }) {
  ...
  useEffect(() => {
    const obtenerArchivos = async () => {
      ...
      const q = query(collection(db, "guias_prestamo"), where("empresaId", "==", empresaSeleccionada.id))
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const archivoRef = ref(storage, `guias_prestamo/${empresaSeleccionada.id}/${archivoSeleccionado.name}`)
      await uploadBytes(archivoRef, archivoSeleccionado)
      const url = await getDownloadURL(archivoRef)

      await addDoc(collection(db, "guias_prestamo"), {
  ...
  const confirmarEliminarArchivo = async () => {
    ...
    try {
      await deleteDoc(doc(db, "guias_prestamo", archivoAEliminar.id))
      ...
      console.error(error)
      setMostrarModalEliminando(false)
      Swal.fire("Error", "No se pudo eliminar la guía de préstamo.", "error")
      setArchivoAEliminar(null)
    }
  }
  ...
  const obtenerTipoArchivo = (nombre) => {
    ...
    if (nombreLower.includes("prestamo") || nombreLower.includes("extintores")) return "prestamo"
    if (nombreLower.includes("devolucion")) return "devolucion"
    if (nombreLower.includes("inventario")) return "inventario"
    if (nombreLower.includes("mantenimiento")) return "mantenimiento"
    return "documento"
  }
  ...
  const obtenerBadgeTipo = (tipo) => {
    switch (tipo) {
      ...
      case "inventario":
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">Inventario</span>
      case "mantenimiento":
        return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-6 mb-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Panel de Guías de Extintores en Préstamo</h1>
            <p className="text-red-100">Gestiona las guías de préstamo de extintores de la empresa</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-6">
          ...
          <div className="relative">
            <textarea
              placeholder="Agrega un comentario sobre esta guía de préstamo..."
              value={comentario}
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {["todos", "prestamo", "devolucion", "inventario", "mantenimiento", "documento"].map((tipo) => (
                <button
                  key={tipo}
      ...
      {archivosOrdenados.length === 0 ? (
        ...
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No hay guías de préstamo</h3>
              <p className="text-gray-500">Esta empresa aún no tiene guías de préstamo subidas.</p>
            </div>
          </div>
        </motion.div>
      ) : (
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="relative">
                      <button
                        ...
                        title="Eliminar guía de préstamo"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path
                            strokeLinecap="round"
      ...
      {archivos.length > 0 && (
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "inventario").length}
              </div>
              <div className="text-sm text-red-700">Inventarios</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "mantenimiento").length}
              </div>
              <div className="text-sm text-red-700">Mantenimiento</div>
            </div>
          </div>
        </motion.div>
      )}
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Guía de préstamo subida correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalExito(false)}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¿Eliminar guía de préstamo?</h3>
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¡Éxito!</h3>
                  <p className="text-sm text-gray-500">Guía de préstamo eliminada correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalEliminado(false)}
...
export default GuiasPrestamoPanel
