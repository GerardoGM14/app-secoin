...
function InformesDetalladosPanel({ empresaSeleccionada }) {
  ...
  useEffect(() => {
    const obtenerArchivos = async () => {
      ...
      const q = query(collection(db, "informes_detallados"), where("empresaId", "==", empresaSeleccionada.id))
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const archivoRef = ref(storage, `informes_detallados/${empresaSeleccionada.id}/${archivoSeleccionado.name}`)
      await uploadBytes(archivoRef, archivoSeleccionado)
      const url = await getDownloadURL(archivoRef)

      await addDoc(collection(db, "informes_detallados"), {
      ...
      console.error(error)
      setMostrarModalCarga(false)
      Swal.fire("Error", "No se pudo subir el informe.", "error")
    } finally {
      setCargando(false)
    }
  }
  ...
  const confirmarEliminarArchivo = async () => {
    ...
    try {
      await deleteDoc(doc(db, "informes_detallados", archivoAEliminar.id))
  ...
  const obtenerTipoArchivo = (nombre) => {
    ...
    if (nombreLower.includes("analisis") || nombreLower.includes("análisis")) return "analisis"
    if (nombreLower.includes("estudio")) return "estudio"
    return "documento"
  }
  ...
  const obtenerBadgeTipo = (tipo) => {
    switch (tipo) {
      case "informe":
        return <span className="px-2 py-1 bg-red-100 text-red-800 text-xs font-medium rounded-full">Informe</span>
      case "reporte":
        return <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">Reporte</span>
      case "analisis":
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">Análisis</span>
      case "estudio":
        return <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">Estudio</span>
      default:
    ...
    >
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-6 mb-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Panel de Informes Detallados</h1>
            <p className="text-red-100">Gestiona los informes y reportes detallados de la empresa</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-6">
          ...
          <div className="relative">
            <textarea
              placeholder="Agrega un comentario sobre este informe..."
              value={comentario}
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {["todos", "informe", "reporte", "analisis", "estudio", "documento"].map((tipo) => (
                <button
                  key={tipo}
        ...
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500">Esta empresa aún no tiene informes detallados subidos.</p>
            </div>
          </div>
        </motion.div>
      ) : (
      ...
      {archivos.length > 0 && (
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "estudio").length}
              </div>
              <div className="text-sm text-red-700">Estudios</div>
            </div>
          </div>
        </motion.div>
      )}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¿Eliminar informe?</h3>
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¡Éxito!</h3>
                  <p className="text-sm text-gray-500">Informe eliminado correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalEliminado(false)}
...
export default InformesDetalladosPanel
