...
function OrdenesCompraPanel({ empresaSeleccionada }) {
  ...
  useEffect(() => {
    const obtenerArchivos = async () => {
      ...
      const q = query(collection(db, "ordenes_compra"), where("empresaId", "==", empresaSeleccionada.id))
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const archivoRef = ref(storage, `ordenes_compra/${empresaSeleccionada.id}/${archivoSeleccionado.name}`)
  ...
  const confirmarEliminarArchivo = async () => {
    ...
    try {
      await deleteDoc(doc(db, "ordenes_compra", archivoAEliminar.id))
  ...
  const obtenerTipoArchivo = (nombre) => {
    ...
    if (nombreLower.includes("orden") || nombreLower.includes("compra")) return "orden"
  ...
  const obtenerBadgeTipo = (tipo) => {
    switch (tipo) {
      case "orden":
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">Orden</span>
      case "factura":
        return <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">Factura</span>
      case "recibo":
        return <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">Recibo</span>
      default:
    ...
    >
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-6 mb-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Panel de Órdenes de Compra</h1>
            <p className="text-red-100">Gestiona las órdenes de compra y documentos relacionados</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-6">
          ...
          <div className="relative">
            <textarea
              placeholder="Agrega un comentario sobre esta orden de compra..."
              value={comentario}
              onChange={(e) => setComentario(e.target.value)}
              className="w-full p-4 border border-gray-300 rounded-xl focus:ring-red-500 focus:border-transparent resize-none transition-all duration-200"
              rows={3}
              maxLength={500}
            />
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {["todos", "orden", "factura", "recibo", "documento"].map((tipo) => (
                <button
                  key={tipo}
            ...
            <select
              ...
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-red-500 focus:border-transparent"
            >
        ...
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500">Esta empresa aún no tiene órdenes de compra subidas.</p>
            </div>
          </div>
        </motion.div>
      ) : (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "orden").length}
              </div>
              <div className="text-sm text-red-700">Órdenes</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "factura").length}
              </div>
              <div className="text-sm text-red-700">Facturas</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "recibo").length}
              </div>
              <div className="text-sm text-red-700">Recibos</div>
            </div>
          </div>
        </motion.div>
      )}
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Orden de compra subida correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalExito(false)}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¿Eliminar orden de compra?</h3>
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¡Éxito!</h3>
                  <p className="text-sm text-gray-500">Orden de compra eliminada correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalEliminado(false)}
...
export default OrdenesCompraPanel
