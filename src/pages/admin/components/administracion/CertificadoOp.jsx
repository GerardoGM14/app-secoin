...
function CertificadoOp({ empresaSeleccionada }) {
  ...
  useEffect(() => {
    const obtenerArchivos = async () => {
      ...
      const q = query(collection(db, "certificados_operatividad"), where("empresaId", "==", empresaSeleccionada.id))
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const archivoRef = ref(storage, `certificados_operatividad/${empresaSeleccionada.id}/${archivoSeleccionado.name}`)
      await uploadBytes(archivoRef, archivoSeleccionado)
      const url = await getDownloadURL(archivoRef)

      await addDoc(collection(db, "certificados_operatividad"), {
  ...
  const confirmarEliminarArchivo = async () => {
    ...
    try {
      await deleteDoc(doc(db, "certificados_operatividad", archivoAEliminar.id))
  ...
  const obtenerTipoArchivo = (nombre) => {
    ...
    if (nombreLower.includes("licencia")) return "licencia"
    if (nombreLower.includes("permiso")) return "permiso"
    if (nombreLower.includes("autorizacion")) return "autorizacion"
    return "documento"
  }
  ...
  const obtenerBadgeTipo = (tipo) => {
    switch (tipo) {
      ...
      case "licencia":
        return <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">Licencia</span>
      case "permiso":
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">Permiso</span>
      case "autorizacion":
        return (
          <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">Autorización</span>
        )
      default:
    ...
    >
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-6 mb-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Panel de Certificados de Operatividad</h1>
            <p className="text-red-100">Gestiona los certificados y permisos de operatividad de la empresa</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-6">
          ...
          <div className="relative">
            <textarea
              placeholder="Agrega un comentario sobre este certificado..."
              value={comentario}
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {["todos", "certificado", "licencia", "permiso", "autorizacion", "documento"].map((tipo) => (
                <button
                  key={tipo}
      ...
      {archivosOrdenados.length === 0 ? (
        ...
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No hay certificados</h3>
              <p className="text-gray-500">Esta empresa aún no tiene certificados de operatividad subidos.</p>
            </div>
          </div>
        </motion.div>
      ) : (
      ...
      {archivos.length > 0 && (
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "licencia").length}
              </div>
              <div className="text-sm text-red-700">Licencias</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "permiso").length}
              </div>
              <div className="text-sm text-red-700">Permisos</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "autorizacion").length}
              </div>
              <div className="text-sm text-red-700">Autorizaciones</div>
            </div>
          </div>
        </motion.div>
      )}
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Certificado subido correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalExito(false)}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">¡Éxito!</h3>
                  <p className="text-sm text-gray-500">Certificado eliminado correctamente</p>
                </div>
              </div>
              <button
                onClick={() => setMostrarModalEliminado(false)}
