...
function GuiasRemisionPanel({ empresaSeleccionada }) {
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      Swal.fire("Éxito", "Guía de remisión subida correctamente.", "success")
  ...
  const eliminarArchivo = async (id, nombre) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteDoc(doc(db, "guias_remision", id))
        setArchivos(archivos.filter((archivo) => archivo.id !== id))
        Swal.fire("Eliminado", "La guía de remisión ha sido eliminada.", "success")
      } catch (error) {
        console.error(error)
