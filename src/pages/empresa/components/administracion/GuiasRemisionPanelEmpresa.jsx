...
function GuiasRemisionPanelEmpresa() {
  ...
  const obtenerArchivos = async (empresaId) => {
    setCargando(true)
    try {
      console.log("Buscando guías para empresaId:", empresaId)

      const q = query(collection(db, "guias_remision"), where("empresaId", "==", empresaId))
      const snap = await getDocs(q)
      const archivosFiltrados = snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }))

      console.log("Guías encontradas:", archivosFiltrados)
  ...
  const obtenerTipoArchivo = (nombre) => {
    ...
    if (nombreLower.includes("transporte") || nombreLower.includes("envio")) return "transporte"
    if (nombreLower.includes("entrega") || nombreLower.includes("recepcion")) return "entrega"
    return "documento"
  }
  ...
  const obtenerBadgeTipo = (tipo) => {
    switch (tipo) {
      ...
      case "entrega":
        return <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">Entrega</span>
      default:
    ...
    >
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-6 mb-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Guías de Remisión</h1>
            <p className="text-red-100">Consulta y descarga tus guías de remisión</p>
          </div>
        </div>
      </div>
        ...
        >
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {["todos", "guia", "transporte", "entrega", "documento"].map((tipo) => (
                <button
                  key={tipo}
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {busqueda || filtroTipo !== "todos" ? "No se encontraron guías" : "No hay guías disponibles"}
              </h3>
              <p className="text-gray-500">
                {busqueda || filtroTipo !== "todos"
                  ? "Intenta cambiar los filtros de búsqueda"
                  : "Aún no tienes guías de remisión disponibles"}
              </p>
            </div>
          </div>
        </motion.div>
      ...
      {archivos.length > 0 && (
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-red-600">
                {archivos.filter((a) => obtenerTipoArchivo(a.nombre) === "entrega").length}
              </div>
              <div className="text-sm text-red-700">Entregas</div>
            </div>
          </div>
        </motion.div>
      )}
...
export default GuiasRemisionPanelEmpresa
